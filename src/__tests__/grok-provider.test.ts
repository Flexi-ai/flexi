...
describe('GrokProvider', () => {
  ...
  beforeEach(() => {
    provider = new GrokProvider(process.env.GROK_API_KEY || 'test-api-key');
  });

  (hasGrokKey ? describe : describe.skip)('getCompletion', () => {
    ...
    test('throws error for invalid API key', async () => {
      const invalidProvider = new GrokProvider('invalid-key');
      const request: AICompletionRequest = {
        messages: [{ role: 'user', content: 'Hello' }],
      };
  ...
  (hasGrokKey ? describe : describe.skip)('getCompletionStream', () => {
    test('yields chunks with correct structure', async () => {
      ...
      expect(firstChunk).toHaveProperty('provider', 'grok');
    });
  ...
  test('returns available models with correct structure', async () => {
    ...
    expect(models.text).toContain('grok-2');
  });
  ...
  test('constructor sets API key', () => {
    ...
    const testProvider = new GrokProvider(testKey);
    // We can't directly test private fields, but we can test the provider was created
    expect(testProvider).toBeInstanceOf(GrokProvider);
  });
});
